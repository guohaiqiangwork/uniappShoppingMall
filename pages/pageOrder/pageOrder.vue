	...
	<view>
		<view class="page_width">
			<view class="margin_top5">
				<view class="top_moudel_pt">剩余时间 {{ timeDom }}</view>
				<view class="top_moudel_pay">
					<view class="font_size30 padding_top3">需支付</view>
					<view class="font_size50 font_weight600">¥{{ payData.payment }}</view>
				</view>
			</view>

			<view class="pay_list">
				<view class="font_size30 font_colorff padding_bottom2 " style="border-bottom: 1px  dashed #CCCCCC;">选择支付方式</view>
				<view @click="switchPay(item.id)" style="margin-top: 30upx;" class="uni-flex display_space  padding_bottom3" v-for="(item, index) in payList" :key="index">
					<view class="uni-flex  width50">
						<view class=""><image :src="item.url" class="left_img" mode=""></image></view>
						<view class="font_size30 font_colorff margin_left5">{{ item.name }}</view>
					</view>
			...
			<view class="pay_btn" @click="goPayResult">确认付款</view>
		</view>
	...
	methods: {
		...
		getPwd: function(val) {
			if (this.setFalg) {
				...
				this.$http
					.post('/api/balance/pay', data, true)
					.then(res => {
						console.log('89');
						if (res.data.code == 200) {
							this.payFalgY = false;
