<template>
	<view>
		...
		<view class="top_moudel_two" style="padding-top: calc(10upx + var(--status-bar-height))" v-if="!titleFalg">
			...
			<!-- 搜索框 -->
			<view class="width50 text_center font_size36">收益详情</view>
			<view class="font_size30 width20 text_right width30 margin_right3 " style="padding-top: 1%;" @click="tomeFalgTab">筛选</view>
		</view>
		<!-- <view class="title_top" v-if="titleFalg"></view> -->
		<view class="uni-flex  background_colorff padding_bottom2 padding_top2 border_bottom" style="padding-top: calc(10upx + var(--status-bar-height))" v-if="titleFalg">
			<view class="width30 text_left margin_left3" style="padding-top: 1%;" @click="goBack">
				<image src="../../static/image/icon/left.png" class="balk_img" mode=""></image>
			</view>
			<!-- 搜索框 -->
		...
		<view class="page_width">
			<view class="border_bottom  " v-for="(item, index) in productList" :key="index">
				<view class="font_size22 font_color99 margin_top3">{{ item.createTime }}</view>
				<view class="uni-flex">
					<view class="width66 uni-flex">
						<view class="font_sise28">{{ item.nickName }}</view>
						<view class="font_colorcc font_size26 margin_left2">{{ item.mobile }}</view>
					</view>
					...
					<view class="width33 text_center" style="margin-top: -5%;">
						<view class="font_size24">获得分润</view>
						<view class="font_sise28 " style="color: #A58747;">{{ item.splitRunAmount }}</view>
					</view>
				</view>
			</view>
