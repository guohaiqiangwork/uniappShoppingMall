		...
		<template v-if="tabIndexT == 0">
			<view class="page_width" style="padding-top: 70upx;">
					...
					<view class="order_list" v-for="(item, index) in applyOrderList" :key="index">
						...
						<view class="uni-flex">
							<view class="uni-flex width50 ">
								<view class="width20"><image :src="item.storeLogo" class="order_listimg" mode=""></image></view>
								<view
									style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"
									class="width80  font_sise28 margin_top2 margin_left5 font_weight700"
								>
									{{ item.storeName }}
								</view>
							</view>
						...
						<view class="" v-if="item.details.length == 1">
							<view class="uni-flex" v-for="(items, index) in item.details" :key="index" @click="goOrder(item.orderId)">
								<view class="width30"><image :src="items.image" class="order_productimg" mode=""></image></view>
								<view class="width75">
									<view class="text_hidden">{{ items.title }}</view>
									<view class="margin_top5 font_size22 font_color99">{{ items.ownSpecMap }}</view>
									<view class="">
						...
						<view class="uni-flex display_right">
							<view class="font_size26 font_color99   margin_top1">共{{ item.goodsNum }}件</view>
						...
						<view class="uni-flex margin_top3 display_right" @click="goOrderDetail(item.orderId)">
							<view class="order_listbtn1" v-if="item.status == 4 || item.status == 6">退换货</view>
							<view class="order_listbtn1 margin_left5" v-if="item.status == 2">退款</view>
						</view>
					</view>
				</template>
				...
				<view v-if="applyOrderList.length == 0" class="text_center margin_top18">
					...
					<view class="font_size28 font_color99 margin_top5">暂无消息~</view>
				</view>
			</view>
		</template>
				...
				<view v-if="applyRecordList.length > 0" class="order_list" v-for="(item, index) in applyRecordList" :key="index">
					...
					<view class="uni-flex" @click="goAfter(item.retId)">
						<view class="uni-flex width50 ">
							...
							<view class="width80  font_sise28 margin_top2 margin_left5 uni_flex_hidden font_weight600">{{ item.storeName }}</view>
						</view>
						<view class="uni-flex font_size26 display_right width50">
					...
					<view class="" v-if="item.returnDetails.length == 1">
						<view class="uni-flex" v-for="(items, index) in item.returnDetails" :key="index">
							...
							<view class="width75">
								<view class="text_hidden">{{ items.title }}</view>
								<view class="margin_top5">{{ items.ownSpecMap }}</view>
								<view class="">
									<text class="font_size22 font_colorbe">¥</text>
									<text class="font_colorbe font_size30 font_weight600">{{ items.price }}</text>
									<text class="font_size22 font_color99">/件</text>
					...
					<!-- 总计 -->
					<view class="uni-flex display_right">
						<view class="font_size26 font_color99   margin_top1">共{{ item.num }}件</view>
						<view class="font_size26 margin_left3  text_right">
							实付款
					...
					<view class="uni-flex margin_top3 display_right">
						<view class="" v-if="item.retStatus == 2 && item.applyServer != 4">
							<view class="order_listbtn1" v-if="!item.logisticsCode" @click="saveLogisticsFalg(item.retId)">填写物流单号</view>
						</view>

						<view class="font_color99 font_size26" v-if="item.logisticsCode">物流单号：{{ item.logisticsCode }}</view>
						<view class="order_listbtn1 margin_left5" @click="getApplyCancel(item.retId)" v-if="item.retStatus == 1 || item.retStatus == 3">取消申请</view>
					</view>
				</view>

				<view v-if="applyRecordList.length == 0" class="text_center margin_top18">
					<image src="../../static/image/default/noMsg.png" class="no_img_msg" mode=""></image>
					<view class="font_size28 font_color99 margin_top5">暂无数据~</view>
				</view>
			</view>
		</template>

		<view v-if="applyRecordList.length > 9 || applyOrderList.length > 9"><uni-load-more :status="status" :content-text="contentText" color="#007aff" /></view>

		<!-- 提示框 -->
		<template v-if="pfalg">
			<view class="moudel_content">
				<view class="product_content_block">
					<view class="font_size34 text_center" style="margin-top: 100upx;">是否取消申请</view>
		...
		<template v-if="logisticsFalg">
			<view class="moudel_content">
				<view class="product_content_block_wu">
					<view class="uni-flex">
						<view class="width33"><image @click="closelogisticsFalg" src="../../static/image/icon/close.png" style="width: 26upx;height: 26upx;" mode=""></image></view>
						<view class="">填写物流单号</view>
					</view>
					...
					<view class="margin_top5"><view class="rightbtn_one" @click="oklogisticsFalg">确认</view></view>
				</view>
			</view>
		</template>
	</view>
...
export default {
	...
	methods: {
		...
		goOrderDetail: function(orderId) {
			...
			this.$http.get('/api/retOrder/checkOrderRefund', data, true).then(res => {
				console.log(JSON.stringify(res));
				if (res.data.code == 200) {
					uni.navigateTo({
