		...
		<view class="page_width padding_top2 padding_bottom3">
			...
			<view class="margin_top3" v-else style="height: 200upx;" @click="goAddAddress('new')">
				...
				<view class="address_order_moudel uni-flex">
					...
					<!-- conter-->
					<view class="width75">
						<view class="uni-flex font_weight600">
							<view class="font_size32">{{ AddressList.receiver }}</view>
							<view class="font_size28 margin_left3 margin_top1">{{ AddressList.mobile }}</view>
						</view>
						<view class="font_size28 text_hidden2">{{ AddressList.address }}</view>
					</view>
					<!-- right -->
					<view class="width10 text_center margin_top8" @click="go"><image src="../../static/image/icon/right.png" class="right_img_wi" mode=""></image></view>
				</view>
			</view>
			...
			<view class="product_moudel_block" v-for="(item, index) in productList" :key="index">
				<view class="uni-flex ">
					<view class=""><image :src="item.storeLogo" class="product_img" mode=""></image></view>
					<view class="font_sise28 margin_top1 margin_left3">{{ item.storeName }}</view>
				</view>

				<template v-if="item.cartResults.length == 1">
					<view class="uni-flex margin_top3">
						<view class="width30"><image :src="item.cartResults[0].image" class="img_product" mode=""></image></view>
						<view class="width65">
							<view class="font_sise28 text_hidden2">{{ item.cartResults[0].title }}</view>
							<view class="font_size22 font_color99 margin_top3">{{ item.cartResults[0].ownSpec }}</view>
						...
						<view class="font_size24 font_color99 margin_top10">*{{ item.cartResults[0].num }}</view>
					</view>
				</template>
					...
					<view class="uni-flex">
						<scroll-view scroll-x="true" class="wrapper">
							...
							<view class="dimg_moudel " v-for="(items, index) in item.cartResults" :key="index">
								<view class=""><image :src="items.image" class="dimg" mode=""></image></view>
			...
			<view class="uni-flex font_size30   list_one_moudel " v-if="invalidList.length > 0">
				<view class="margin_left3 width40">失效商品清单</view>
				<view class="margin_right3 uni-flex width60" @click="goInvalidProduct('s')">
					<view class="width90 text_right">共{{ invalidList.length }}件</view>
					<view class="margin_top1 margin_left2"><image src="../../static/image/icon/right.png" class="right_img_wi" mode=""></image></view>
				</view>
			</view>

			<!-- 优惠卷 -->
			<view class="uni-flex font_size30   list_one_moudel " v-if="discountMoney.balance">
				<view class="margin_left3 width40">优惠劵</view>
				<view class="margin_right3 uni-flex width60">
					<view class="width90 text_right" @click="openCoupon">
						<text class="font_size22">可用￥</text>
						{{ discountMoney.balance }}
					</view>
					<view class="margin_top1 margin_left2"><image src="../../static/image/icon/right.png" class="right_img_wi" mode=""></image></view>
				</view>
			</view>
			...
			<view class="bottom_proudel uni-flex">
				<view class="font_size28 width60 margin_top3 margin_left3">
					...
					<text class="font_colorbe font_weight600">{{ actualPayment }}</text>
				</view>
				<view class="right_m margin_left3" @click="goBuy">去支付</view>
			</view>
		</view>
				...
				<view class="list_moudel">
					<view class="uni-flex border_bottom padding_bottom2">
						<view class="width80 uni-flex">
							<view class="margin_top2" @click="closeCoupon"><image src="../../static/image/icon/close.png" class="close_img" mode=""></image></view>
							<view class="font_size32 margin_left5">优惠券</view>
						</view>
						<view class="font_size26 font_color66 width20 text_right"><!-- 使用说明 --></view>
					</view>
								...
								<view class="uni-flex">
									<view class="width35 text_center">
										<view class="font_size60 ">
											<text class="font_size28">¥</text>
											{{ discountMoney.amount }}
										</view>
										<view class="font_size20 ">剩余金额{{ discountMoney.balance }}</view>
									</view>
									<view class="text_center width60 margin_top5">
										<view class="font_size26">全额优惠劵</view>
										<view class="" style="border-top: 1upx solid #333333; width: 30%;margin-left: 35%;"></view>
										<view class="font_size20">{{ discountMoney.createTime }}</view>
									</view>
								</view>
							</view>

							<view v-else class="text_center">暂无可使用优惠卷</view>
						</view>
					</scroll-view>
					<!-- <view class="btn_bj">
		...
		<template v-if="changeList">
			<view class="couponMoudel">
				<view class="change_moudel">
					<view class="font_size32 text_center border_bottom padding_bottom3">以下商品信息发生变动</view>
					<scroll-view scroll-y="true" style="max-height: 700upx;">
						<view class="uni-flex product_moudel_list border_bottom" v-for="(item, index) in dataInvalidList" :key="index">
							...
							<view class="width70">
								<view class="font_sise28 font_weight600 text_hidden2">{{ item.title }}</view>
								...
								<view class="font_size24 font_colorbe margin_top3" style="color: #BE8100;">商品库存仅剩 {{ item.goodsNum }} 个</view>
							</view>
						</view>
					</scroll-view>
					<view class="chang_bottom uni-flex">
						<view class="chang_bottom1" @click="closeMoudel">返回购物车确认</view>
						<view class="chang_bottom2" @click="goBuy">继续支付订单</view>
					</view>
				</view>
			</view>
		</template>
		...
		<template v-if="pfalg">
			<view class="moudel_content">
				<view class="product_content_block">
					<view class="font_size34 text_center" style="margin-top: 100upx;">确定使用优惠券全额抵扣？</view>
					<view class="uni-flex " style="margin-top: 80upx;">
						<view class="leftbtn" @click="closemoudel">取消</view>
						<view class="rightbtn" @click="goPageOrder">确认</view>
					</view>
				</view>
			</view>
		</template>
...
export default {
	...
	methods: {
		...
		getAddress: function() {
			...
			this.$http
				.get('/api/address/find', data, true)
				.then(res => {
					console.log(res);
					if (res.data.code == 200) {
		...
		goPageOrder: function(item) {
			...
			this.productList.map(function(item) {
				...
				item.cartResults.map(function(items) {
					a.orderDetais.push({
						cartId: items.id,
