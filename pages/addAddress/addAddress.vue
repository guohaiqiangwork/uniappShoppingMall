					...
					<view class="uni-flex">
						<view class="width75" @click="goShop(item)">
							<view class="uni-flex font_size30 font_weight600">
								...
								<view class="margin_left2">{{ item.mobile }}</view>
		...
		<block v-if="!newAddress">
			...
			<view class="form_body">
				...
				<view class="form_list">
					...
					<view class="list_right">
						<input class="list_input" @input="keyPhone" maxlength="11" placeholder="请填写收货人手机号码" placeholder-style="color:#999999" />
					</view>
				</view>
				<view class="form_list">
					...
					<view class="list_right uni-flex">
						...
						<view class="list_input margin_left3" @tap="handleTap('picker1')">
							<view class="font_color99">{{ province }}</view>
							<lb-picker ref="picker1" mode="selector" :list="provinceList" @confirm="handleConfirm"></lb-picker>
						</view>
						<view class="list_input margin_left3" @tap="handleTap('picker2')">
							<view class="font_color99">{{ city }}</view>
							<lb-picker ref="picker2" mode="selector" :list="cityList" @confirm="handleConfirmCity"></lb-picker>
						</view>
						<view class="list_input margin_left3" @tap="handleTap('picker3')">
							<view class="font_color99">{{ area }}</view>
							<lb-picker ref="picker3" mode="selector" :list="areaList" @confirm="handleConfirmArea"></lb-picker>
						</view>
			...
			<!-- <view class=" background_colorff">
				<view class="uni-flex bottom_list_s">
					<view class="bottom_list_left">设置默认地址</view>
			...
			<view class="bottom_add">
				<!-- @click="addAddr" -->
				<view class="fixed_btm" @click="addAddr">保存</view>
			</view>
			<!-- 地址框 -->
...
export default {
	...
	methods: {
		...
		addAddr: function() {
			...
			this.$http
				.post('/api/address/add', keyword, true)
				.then(res => {
					uni.hideLoading();
		...
		getAddressList: function() {
			...
			this.$http
				.get('/api/address/list', dataA, true)
				.then(res => {
					if (res.data.code == 200) {
