			...
			<view class="top_money">
				<view class="uni-flex border_bottom padding_bottom3">
					...
					<view class="uni-flex width65" @click="open_m">
						...
						<view class="font_size30 margin_left3">
							...
							<view class="font_size26 font_color99">2小时内到账</view>
						</view>
					</view>
					<view class="width10" @click="open_m"><image src="../../static/image/icon/rightb.png" mode="" class="right_i"></image></view>
				</view>
				<view class="font_size30 margin_top8" @click="overMoney">提现金额</view>
				...
				<view class="border_bottom  margin_top5 font_size50 uni-flex">
					...
					<input
						...
						@input="keyMoney"
						placeholder="请输入金额"
						placeholder-style="color:#484848;font-size:30upx"
						style="margin-top: 2%;"
					/>
				</view>

				<view class="uni-flex font_size26 margin_top3">
					<view class=" font_color99">余额￥{{ money }}</view>
					<view class="font_color99 margin_left2" v-if="taxationData.headFee != 0">
						手续费:
						<text>{{ taxationData.headFee }}</text>
					</view>
					<view class="font_color99 margin_left2" v-if="taxationData.taxes != 0">
						税费:
						<text>{{ taxationData.taxes }}</text>
					</view>
					<view class="margin_left3" @click="overMoney">全部提现</view>
				</view>

				<view class="w_btn" @click="goW">提现</view>
			</view>
		</view>
		...
		<template v-if="backFalg">
			<view class="moudel_content">
				<view class="moudel_content_w">
					<view class="uni-flex">
						<view class="width33" @click="close_m"><image src="../../static/image/icon/close.png" class="img_left" mode=""></image></view>
						<view class="text_center font_size32">选择到账银行卡</view>
					</view>
					<scroll-view scroll-y="true">
						<view @click="selectBank(item)" class="uni-flex border_bottom padding_bottom3 padding_top2" v-for="(item, index) in bankList" :key="index">
							<view class="width20 text_center"><image :src="item.imgUrl" class="listimg" mode=""></image></view>
							<view class="font_size32 width50">{{ item.bankName }}</view>
							<view class="width20 text_right" v-if="bankF == item.id"><image src="../../static/image/icon/dui.png" mode="" class="mi"></image></view>
						</view>

						<view class="uni-flex border_bottom padding_bottom3 padding_top2" v-if="bankList.length < 10">
							...
							<view class="font_size32 width50">添加银行卡</view>
			...
			<view class="moudel_content">
				<view :class="setFalg ? 'content_block' : 'content_block_pay'">
					<view class="uni-flex uni-row margin_left3 margin_top3">
						<view class="width30" @click="closeMoudel"><image src="../../static/image/icon/close.png" mode="" class="moudel_img"></image></view>
					...
					<view class="font_size30 font_color66 text_center" v-if="!setFalg">提现</view>
					<view class="font_size60 font_color66 text_center" v-if="!setFalg">￥{{ tiXiMoney }}</view>
...
export default {
	...
	methods: {
		...
		// 去提现结果
		goW: function() {
			this.tiXiMoney = this.moneyValue;
			...
			this.$http
				.get('/api/account/isSetPassword', data, true)
				.then(res => {
					if (res.data.code == 200) {
						console.log('99');
						if (res.data.data) {
		...
		getSetPassword: function() {
			...
			this.$http
				.post('/api/account/setPassword', data, true)
				.then(res => {
					if (res.data.code == 200) {
						uni.showToast({
							title: '操作成功',
		...
		getPwd: function(val) {
			...
			if (this.setFalg) {
				...
				var data = {
					mbId: uni.getStorageSync('userId'),
					password: val,
					amount: this.tiXiMoney,
					bankId: this.bankF
				};
				this.$http
					.post('/api/withdraw/apply', data, true)
					.then(res => {
						if (res.data.code == 200) {
							console.log(JSON.stringify(res));
							this.payFalg = false;
