<template>
	<view>
		...
		<view class="uni-flex  background_colorff padding_bottom2 padding_top2" style="padding-top: calc(20upx + var(--status-bar-height))">
			...
			<view @click="goSearchList" class="font_size28 text_center width15 " style="padding-top: 1%;">搜索</view>
		</view>
		...
		<view class="background_colorf8 padding_top3 padding_bottom3">
			<view class="page_width" v-if="searchAll.length > 0">
				<view class="uni-flex display_space">
					<view class="font_size28">历史搜索</view>
					<view class="" @click="closeSearchAll"><image src="../../static/image/icon/sdelect.png" style="width: 29upx;height: 28upx;" mode=""></image></view>
				</view>
				<view class="padding_bottom3">
					<view class="tab_item" @click="assignment(item)" v-for="(item, index) in searchAll" :key="index">{{ item }}</view>
				</view>
			</view>
			...
			<view class="page_width margin_top3">
				<view class="font_size28">热门搜索</view>
				<view class="uni-flex" @click="assignment(item.label)" v-for="(item, index) in list" :key="index">
					<view class="list_left margin_right3" v-if="item.type == 1">爆</view>
					<view class="list_left margin_right3" style="background-color: #7EB546;" v-if="item.type == 2">推</view>
		...
		<template v-if="pfalg">
			<view class="moudel_content">
				<view class="product_content_block">
					<view class="font_size34 text_center" style="margin-top: 100upx;">确认要删除历史搜索吗？</view>
