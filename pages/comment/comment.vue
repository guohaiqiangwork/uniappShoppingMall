		...
		<view class="page_width">
			...
			<template>
				<view class="uni-flex list_moudel_search">
					<view class="width25"><image :src="orderDetail.image" class="list_img" mode=""></image></view>
					<view class="width66">
						<view class="text_hidden font_weight600">{{ orderDetail.title }}</view>
						<view class="margin_top8"><uni-rate size="24" active-color="#B99445" :value="rateNumber" @change="onChange"></uni-rate></view>
					</view>
				</view>
			</template>
			...
			<view class="bo_moudel">
				<view class="font_sise28">详细评价</view>
				<view class="margin_top3"><textarea value="" @input="getTextValue" placeholder="请填写内容" class="text_moudel" /></view>
			...
			<view class="btn_comment" @click="openMoudel">提交申请</view>
		</view>
		...
		<template v-if="pfalg">
			<view class="moudel_content">
				<view class="product_content_block">
					<view class="font_size34 text_center" style="margin-top: 100upx;">提交后无法修改，是否确定提交</view>
					<view class="uni-flex " style="margin-top: 80upx;">
						<view class="leftbtn" @click="closemoudel">取消</view>
						<view class="rightbtn" @click="saveComment">确认</view>
					</view>
				</view>
			</view>
		</template>
		...
		delectUpload: function(index) {
			...
			if (this.imgList[index].url == '../../static/image/beij/uplodel.png') {
				...
				if (this.imgList.length == 3) {
					if (this.imgList[2].url == '../../static/image/beij/uplodel.png') {
						this.imgList.splice(index, 1);
						this.imgListData.splice(index, 1);
					} else {
						this.imgList[2].url = '../../static/image/beij/uplodel.png';
						this.imgListData.splice(index, 1);
					}
				} else {
					this.imgList.splice(index, 1);
