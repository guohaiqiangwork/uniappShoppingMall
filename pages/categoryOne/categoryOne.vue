<template>
	<view>
		...
		<!-- 头部搜索 -->
		<view
			class="uni-flex  background_colorff padding_bottom2 padding_top2"
			style="padding-top: calc(20upx + var(--status-bar-height));position: fixed;width: 100%;z-index: 99;"
			v-if="titleFalg"
		>
			...
			<view class="uni-flex">
				...
				<view class="width75 background_colorff" style="min-height: 1200upx;margin-left: 25%;">
					<view class="uni-flex  _right_moudel" v-for="(item, index) in rightList" :key="index">
						<view class="" @click="goProductDetails(item.id)" style=""><image :src="item.goodsDetail.images" class="_right_img" mode=""></image></view>
						<view class="margin_left3 width70">
							<view class="font_size26  text_hidden " @click="goProductDetails(item.id)">{{ item.title }}</view>
							<view class="font_size22 font_color99 text_hidden margin_top3" @click="goProductDetails(item.id)">{{ item.subTitle }}</view>
