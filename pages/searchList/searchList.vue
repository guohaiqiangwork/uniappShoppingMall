<template>
	<view>
		...
		<view class="" v-if="true" style="position: fixed;width: 100%; background-color: #ffffff;z-index: 9;padding-top: 30upx;">
			<!-- 头部搜索 -->
			<view class="uni-flex  background_colorff padding_bottom2 padding_top2" style="padding-top:var(--status-bar-height);">
				<view class="width10 text_center" style="padding-top: 1%;" @click="goBack"><image src="../../static/image/icon/left.png" class="balk_img" mode=""></image></view>
				<!-- 搜索框 -->
				<view class="uni-flex top_content">
					<view class="searce_left" style="width: 7%;"><image src="../../static/image/icon/search.png" class="searce_width" mode=""></image></view>
			...
			<view class="background_colorff">
				...
				<view class="padding_top3 padding_bottom3" v-if="this.tabIndex == 0">
					<view @click="tabSwichThree(index)" class="item_tab_three" v-for="(item, index) in tabListThree" :key="index" :style="index == 2 ? 'border:none' : ''">
						<view :class="twoTab == index ? 'font_weight700' : ''">{{ item.name }}</view>
		...
		<!-- 列表 -->
		<view class="page_width" :style=" tabIndex == 0 ?  'paddingTop: 340upx;' : 'paddingTop:240upx'" >
			<!-- 商品 -->
			<template v-if="tabIndex == 0" >
				<view class="uni-flex list_moudel_search"  v-for="(item, index) in queryGoodsList" :key="index">
					<view class="width30" @click="goTodetails(item)"><image :src="item.goodsDetail.images" class="list_img" mode=""></image></view>
					<view class="width66">
						<view class="margin_top5 text_hidden font_weight600" @click="goTodetails(item)">{{ item.title }}</view>
						<view class="uni-flex display_space margin_top8">
							<view class="font_size22 font_color66 ">
								<text class="font_colorbe" style="font-size: 28upx;">¥{{ item.goodsDetail.price }}</text>
								/件
							</view>
							<view class="margin_right3" @click="addShopCard(item)"><image src="../../static/image/icon/shopCard.png" class="image_list_s" mode=""></image></view>
						</view>
					</view>
				</view>
			...
			<template v-else>
				<view class="">
					<view class="shop_moudel uni-flex" v-for="(item, index) in queryStoreList" :key="index">
						<view class="width15 text_center" ><image :src="item.shoreLogo" class="shop_moudel_img" style="border-radius: 12upx;" mode=""></image></view>
						<view class="font_size28 margin_top1 width65">{{ item.storeName }}</view>
						<view class="width20"><view class="shop_moudel_btn" @click="goToShop(item.id)">进店</view></view>
					</view>
							
					<view v-if="queryStoreList.length == 0" class="text_center margin_top18">
						<image src="../../static/image/default/noProduct.png" class="no_img_product" mode=""></image>
						<view class="font_size28 font_color99 margin_top5">暂无相关店铺~</view>
					</view>
				</view>
			...
			<view v-if="queryStoreList.length > 9"><uni-load-more :status="status" :content-text="contentText" color="#999999" /></view>
		</view>
