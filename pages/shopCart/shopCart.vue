	...
	<view>
		...
		<view class="" style="padding-bottom: 120upx;">
			...
			<view class="product_moudel" v-for="(item, index) in productList" :key="index">
				<!-- 标题 -->
				<view class="uni-flex" style="padding-top: 3%;">
					<view class="margin_left3">
						<checkbox-group @change="allChoose('all', index)">
							<label><checkbox value="all" :class="{ checked: item.allChecked }" :checked="item.allChecked ? true : false"></checkbox></label>
						</checkbox-group>
					</view>
					<view class=""><image :src="item.storeLogo" mode="" class="shopTitle"></image></view>
					<view class="font_size28 margin_left3 " style="margin-top: 1%;">{{ item.storeName }}</view>
				</view>
				<!-- 内容 -->
				<view class="uni-flex margin_top3 padding_bottom2" v-for="(items, index) in item.cartResults" :key="index">
					<view class="margin_top8 margin_left3">
						<checkbox-group @change="changeCheckbox(item.storeId, index)">
							<label><checkbox value="String(item)" :class="{ checked: items.oneChecked }" :checked="item.oneChecked ? true : false"></checkbox></label>
						</checkbox-group>
					</view>
					<view class=""><image :src="items.image" class="product_moudel_img" mode=""></image></view>
					<view class="width60 margin_left2">
						<view class="font_size28 text_hidden">{{ items.title }}</view>
						<view class="font_size22 font_color99" style="margin-top: 6%;">{{ items.ownSpec }}</view>
						<view class="uni-flex display_space">
							...
							<view class="uni-flex num_moudel">
								<view class="num_moudel_left font_color99" @click="numStatistics(item.id, items.id, items.num, '', items)">-</view>
								<view class="num_moudel_center">{{ items.num > items.stock ? items.stock : items.num }}</view>
								<view class="num_moudel_right" @click="numStatistics(item.id, items.id, items.num, 'add', items)">+</view>
							</view>
						</view>
					</view>
				</view>
			...
			<!-- 失效商品 -->
			<view class="product_moudel" style="margin-top: 40upx;" v-if="invalidList.length > 0">
				<view class="text_center font_size28 padding_top3 font_weight600">失效商品({{ moreInvalidList.length }} 件)</view>

				<view class="uni-flex margin_top5" style="background-color: #FFFFFF;opacity: 0.5;" v-for="(items, index) in invalidList" :key="index">
					<view class="margin_top8 margin_left3 invalid_moudel">失效</view>
					<view class="margin_left3"><image :src="items.image" class="product_moudel_img" mode=""></image></view>
					<view class="width60 margin_left2">
						<view class="font_size28 font_weight600">{{ items.title }}</view>
						<view class="font_size22 font_color99">{{ items.ownSpec }}</view>
						<view class="uni-flex display_space">
							<view class="font_size28 " style="color: #BE8100;">
								¥{{ items.price }}
								<text class="font_size22 font_color99">/件</text>
				...
				<view class="uni-flex padding_bottom3 margin_top3">
					<view class="width70"><view class="text_center font_size24 delect_invalid_moudel " @click="emptyShopCardInval">清除全部失效商品</view></view>

					<view class="width20" v-if="moreInvalidList.length > 1">
						<view class=" text_right font_size24" v-if="invalidList.length == 1" @click="openinvalidList">
							展开
							<image class="margin_left3" src="../../static/image/icon/sj.png" style="width: 10upx;height: 7upx;" mode=""></image>
						</view>
					</view>

					<view class="width20 text_left font_size24" v-if="invalidList.length != 1" @click="closeinvalidList">
						关闭
						<image class="margin_left3" src="../../static/image/icon/sj.png" style="width: 10upx;height: 7upx;" mode=""></image>
					</view>
				</view>
			...
			<view v-if="productList.length == 0 && invalidList.length == 0" class="text_center margin_top18">
				<image src="../../static/image/default/noShop.png" class="no_img_shop" mode=""></image>
				<view class="font_size28 font_color99 margin_top5">空空如也购物车~</view>
			</view>
		</view>
			...
			<view class="uni-flex bottom_z_left">
				<view class="bottom_z_left_check">
					<checkbox-group @change="qAllChoose">
						<label><checkbox :class="{ checked: quanAllChecked }" :checked="quanAllChecked ? true : false"></checkbox></label>
					</checkbox-group>
				</view>
				<view class="font_size26 " style="color: #6F6F6F;margin-left: -5%;margin-top: 2%;">全选</view>
			</view>
			<view class="bottom_z_center font_size28 font_color33">
				总价：
				<text class="font_colorde font_weight600">¥{{ totalPrice }}</text>
			</view>
			<view class="bottom_z_right" @click="goConfirmOrder">去结算({{ totalPriceNum }})</view>
		</view>
		...
		<template v-if="pfalg">
			<view class="moudel_content">
				<view class="product_content_block">
					<view class="font_size34 text_center" style="margin-top: 100upx;">确认要删除商品吗？</view>
					<view class="uni-flex " style="margin-top: 80upx;">
						<view class="leftbtn" @click="closemoudel">取消</view>
						<view class="rightbtn" @click="okMoudel">确认</view>
					</view>
				</view>
			</view>
		</template>
...
export default {
	...
	methods: {
		...
		// 数字框处理
		numStatistics: function(shopCartId, shopId, num, type, dataItem) {
		...
		// 多选复选框改变事件
		changeCheckbox: function(shopCartId, index) {
			...
			for (let item of this.productList) {
				if (shopCartId == item.storeId) {
					...
					console.log(item.cartResults[listIndex].oneChecked);
